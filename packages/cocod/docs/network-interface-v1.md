...
Cocod bounds each stream queue and disconnects a consumer when another invalidation would overflow
it. After reconnecting, consumers refetch canonical resources. A stream also closes when its Coco
Session stops or is replaced, so it cannot silently remain subscribed to a disposed session.
Cocod periodically revalidates the stream's original Client Credential and closes the stream when
that credential is no longer authorized, including after host-local credential rotation. Session
and authorization checks run at most five seconds apart.
