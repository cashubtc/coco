...
test('closes an open event stream after its credential is rotated', async () => {
  ...
  // A keepalive accepted before rotation may already be queued. Drain it and observe EOF.
  try {
    for (let i = 0; ; i++) {
...
function createRoutes(
  ...
  session?: { manager: FakeManager; state: 'running' | 'stopping' | 'stopped' },
  ...
  const runtime = {
    getStatus: () => ({
      ...status,
      cocoSession: { ...status.cocoSession, state: session?.state ?? 'running' },
    }),
    getRunningSession: () => ({
...
for (const transition of ['stopping', 'stopped', 'replaced'] as const) {
  test(`closes event streams when the originating Coco Session is ${transition}`, async () => {
    const credential = await createCredential();
    const manager = new FakeManager();
    const session: { manager: FakeManager; state: 'running' | 'stopping' | 'stopped' } = {
      manager,
      state: 'running',
    };
    const routes = createRoutes(
      ...
      { eventAuthorizationRevalidationIntervalMs: 5 },
      session,
    );
    const response = await routes['/v1/events']!.GET!(
      new Request('http://localhost/v1/events', {
    ...
    const reader = response.body!.getReader();
    try {
      await readSseBlock(reader);
      if (transition === 'replaced') session.manager = new FakeManager();
      else session.state = transition;
      // Even without any subsequent Wallet event, the stream must end and release its Manager.
      let done = false;
      for (let i = 0; i < 3 && !done; i++) {
        const next = await Promise.race([
          reader.read(),
          Bun.sleep(200).then(() => ({ done: false, timeout: true })),
        ]);
