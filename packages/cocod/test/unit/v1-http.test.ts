...
for (const type of ['mint', 'melt'] as const) {
  test(`${type} Quote creation returns a stable client error for an unavailable Mint`, async () => {
    const credential = await createCredential();
    const create = mock(async () => {
      throw new UnknownMintError('https://mint.example.com');
    });
    const routes = createWalletTestRoutes(
      { quotes: { [type]: { create } } },
      credential.credentials,
    );
    const response = await routes[`/v1/quotes/${type}`]!.POST!(
      new Request(`http://localhost/v1/quotes/${type}`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${credential.plaintext}`,
          'Content-Type': 'application/json',
        ...
        body: JSON.stringify(
          type === 'mint'
            ? { method: 'bolt11', amount: '21', unit: 'sat' }
            : { method: 'bolt11', invoice: 'lnbc21' },
        ),
      }),
    );
    ...
    expect(response.status).toBe(409);
    expect(await response.json()).toMatchObject({
      error: { code: 'mint_unavailable', retryable: false },
    });
  });
  for (const unit of ['', '   ']) {
    test(`${type} Quote creation rejects blank unit ${JSON.stringify(unit)} before calling Coco`, async () => {
      const credential = await createCredential();
      const create = mock(() => {
        throw new Error('must not reach Coco');
      });
      const routes = createWalletTestRoutes(
        { quotes: { [type]: { create } } },
        credential.credentials,
      ...
      const response = await routes[`/v1/quotes/${type}`]!.POST!(
        new Request(`http://localhost/v1/quotes/${type}`, {
          ...
          body: JSON.stringify(
            type === 'mint'
              ? { method: 'bolt11', amount: '21', unit }
              : { method: 'bolt11', invoice: 'lnbc21', unit },
          ),
        }),
      );
      expect(response.status).toBe(400);
...
test('Send preparation maps typed domain validation to a safe client error', async () => {
  const credential = await createCredential();
  const prepare = mock(async () => {
    throw new ProofValidationError('sensitive mint diagnostic');
  });
  const routes = createWalletTestRoutes({ ops: { send: { prepare } } }, credential.credentials);
  const response = await routes['/v1/operations/send']!.POST!(
    new Request('http://localhost/v1/operations/send', {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${credential.plaintext}`,
        'Content-Type': 'application/json',
  ...
  expect(JSON.parse(body)).toMatchObject({ error: { code: 'invalid_request' } });
  expect(body).not.toContain('sensitive mint diagnostic');
});
