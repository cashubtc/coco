...
interface MintUrlRepairDb {
  all<T = unknown>(sql: string, params?: unknown[]): Promise<T[]>;
  get<T = unknown>(sql: string, params?: unknown[]): Promise<T | undefined>;
  run(sql: string, params?: unknown[]): Promise<{ lastID: number; changes: number }>;
  transaction<T>(fn: (tx: MintUrlRepairDb) => Promise<T>): Promise<T>;
}
...
const MINT_URL_STORAGE_TABLES: MintUrlStorageTable[] = [
  'coco_cashu_counters',
  'coco_cashu_proofs',
  'coco_cashu_mint_quotes',
  'coco_cashu_melt_quotes',
...
async function getCanonicalMintUrls(db: MintUrlRepairDb): Promise<Set<string>> {
...
export async function detectSqliteLikeMintUrlStorageIssues(
  db: MintUrlRepairDb,
  normalizeMintUrl: NormalizeMintUrl,
): Promise<MintUrlStorageCheckReport> {
  return db.transaction(async (tx) => {
    const canonicalMintUrls = await getCanonicalMintUrls(tx);
    return createCheckReport(await collectIssues(tx, canonicalMintUrls, normalizeMintUrl));
  });
}
