...
export async function runMintOperationRepositoryContract(
  ...
  describe('MintOperationRepository contract', () => {
    ...
    it('loads mint operations without ownership metadata as standalone', async () => {
      const { repositories, dispose } = await options.createRepositories();
      try {
    ...
    it('assigns a Mint Swap parent when the expected state matches and the operation is unparented', async () => {
      const { repositories, dispose } = await options.createRepositories();
      try {
    ...
    it('rejects conditional Mint Swap claims when state or current parent does not match', async () => {
      const { repositories, dispose } = await options.createRepositories();
      try {
    ...
    it('rolls back conditional Mint Swap claims with the surrounding repository transaction', async () => {
      const { repositories, dispose } = await options.createRepositories();
      try {
    ...
    it('persists reusable BOLT12 mint quote data', async () => {
      ...
      try {
        ...
        expect(amountless.amountIssued.equals(Amount.from(0))).toBe(true);
      } finally {
        await dispose();
      }
    });
...
export async function runMeltOperationRepositoryContract(
  ...
  describe('MeltOperationRepository contract', () => {
    ...
    it('preserves Mint Swap ownership through stale regular melt state updates', async () => {
      const { repositories, dispose } = await options.createRepositories();
      try {
        const init = {
