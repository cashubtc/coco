...
describe('PaymentRequestService', () => {
  ...
  it.each([
    { mints: [testMintUrl], mintsPreferred: true },
    { supportedMethods: [{ method: 'bolt11', fee: 1 }] },
  ])('rejects fulfillment features Coco cannot account for (%p)', async (options) => {
    const paymentRequest = new PaymentRequest({
      amount: 10,
      unit: 'sat',
      mints: options.mints ? [...options.mints] : undefined,
      mintsPreferred: options.mintsPreferred,
      supportedMethods: options.supportedMethods ? [...options.supportedMethods] : undefined,
    });
    await expect(service.parse(paymentRequest.toEncodedCreqB())).rejects.toThrow('not supported');
    expect(mockSendOperationService.init).not.toHaveBeenCalled();
  });

  it('ignores an advisory mint flag when the request has no mint list', async () => {
    const paymentRequest = new PaymentRequest({ amount: 10, unit: 'sat', mintsPreferred: true });
    const resolved = await service.parse(paymentRequest.toEncodedCreqB());
    expect(resolved.amount?.toString()).toBe('10');
  });
