...
export const executeExactSend = defineTransition<ExecuteExactSendInput, ExecuteExactSendResult>(
  async (tx, input) => {
...
export const beginSendExecution = defineTransition<
  BeginSendExecutionInput,
  BeginSendExecutionResult
>(async (tx, input) => {
  const current = await tx.sendOperations.getById(input.operationId);
...
export const applySendResult = defineTransition<ApplySendResultInput, ApplySendResult>(
  async (tx, input) => {
    const current = await tx.sendOperations.getById(input.operationId);
    if (!current) {
      throw new SendOperationConflictError(input.operationId, 'Send operation not found');
...
export const completePendingSend = defineTransition<
  CompletePendingSendInput,
  CompletePendingSendResult
>(async (tx, input) => {
  const current = await tx.sendOperations.getById(input.operationId);
  ...
  if (inputBySecret.size !== current.inputProofSecrets.length) {
    throw new ProofValidationError('Cannot complete Send operation: missing input proof metadata');
  }
...
export const cancelPreparedSend = defineTransition<
  CancelPreparedSendInput,
  CancelPreparedSendResult
>(async (tx, input) => {
  const current = await tx.sendOperations.getById(input.operationId);
...
export const failSendExecution = defineTransition<FailSendExecutionInput, FailSendExecutionResult>(
  async (tx, input) => {
    const current = await tx.sendOperations.getById(input.operationId);
    if (!current) {
      throw new SendOperationConflictError(input.operationId, 'Send operation not found');
...
export const beginSendReclaim = defineTransition<BeginSendReclaimInput, BeginSendReclaimResult>(
  async (tx, input) => {
    const current = await tx.sendOperations.getById(input.operationId);
...
export const completeSendReclaim = defineTransition<
  CompleteSendReclaimInput,
  CompleteSendReclaimResult
>(async (tx, input) => {
  const current = await tx.sendOperations.getById(input.operationId);
...
export const claimSendRecovery = defineTransition<ClaimSendRecoveryInput, ClaimSendRecoveryResult>(
  async (tx, input) => {
    const current = await tx.sendOperations.getById(input.operationId);
    if (
      !current ||
...
export const recoverLegacyExactSend = defineTransition<
  RecoverLegacyExactSendInput,
  RecoverLegacyExactSendResult
>(async (tx, input) => {
  const current = await tx.sendOperations.getById(input.operationId);
...
export const cleanupLegacySendInit = defineTransition<string, CleanupLegacySendInitResult>(
...
export const cleanupOrphanedSendReservations = defineTransition<
  void,
  CleanupOrphanedSendReservationsResult
>(async (tx) => {
