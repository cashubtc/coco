...
export class MemoryMintSwapOperationRepository implements MintSwapOperationRepository {
  ...
  async create(operation: MintSwapOperation): Promise<void> {
    ...
    this.assertUniqueChildOwnership(operation);
  ...
  async getDue(now: number, limit: number): Promise<MintSwapOperation[]> {
    assertNonNegativeSafeInteger(now, 'Due time');
    assertNonNegativeSafeInteger(limit, 'Due limit');
    return this.sorted((operation) => isMintSwapOperationDue(operation, now), true).slice(0, limit);
  }
  ...
  private assertUniqueChildOwnership(candidate: MintSwapOperation): void {
    for (const operation of this.operations.values()) {
  ...
  private sorted(
    ...
    return Array.from(this.operations.values())
      ...
      .sort((left, right) => {
        if (dueOrder) {
          const due =
            (getMintSwapOperationDueAt(left) ?? 0) - (getMintSwapOperationDueAt(right) ?? 0);
          if (due !== 0) return due;
        }
