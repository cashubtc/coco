...
export async function ensureSchema(db: IdbDb): Promise<void> {
  ...
  // Version 32: Persist canonical Mint Quote Accounting as first-class fields.
  db.version(32)
    .stores({
      coco_cashu_mints: '&mintUrl, name, updatedAt, trusted',
      coco_cashu_keysets: '&[mintUrl+id], mintUrl, id, updatedAt, unit',
  ...
  // Version 34: Add monotonic Send operation revisions for conditional transitions.
  db.version(34)
    .stores({
      coco_cashu_mints: '&mintUrl, name, updatedAt, trusted',
      coco_cashu_keysets: '&[mintUrl+id], mintUrl, id, updatedAt, unit',
  ...
  // Version 35: Add dormant Mint Swap parent persistence.
  db.version(35).stores({
    ...
    coco_cashu_mint_swap_operations:
      '&id, state, revision, nextAttemptAt, createdAt, &[sourceQuoteMintUrl+sourceQuoteMethod+sourceQuoteId], &[destinationQuoteMintUrl+destinationQuoteMethod+destinationQuoteId], &sourceOperationId, &destinationOperationId, [state+revision], [state+nextAttemptAt+createdAt+id]',
  });
}
