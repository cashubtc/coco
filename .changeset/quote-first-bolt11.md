...
Core now exposes `ops.mint.createQuote()` and supports preparing BOLT11 mint
operations against an existing canonical quote ID while preserving the existing
`prepare({ amount })` compatibility wrapper. Mint quote records are keyed by
normalized `(mintUrl, method, quoteId)`, and mint operation quote lookups now
return all sibling operations for the full quote identity.
